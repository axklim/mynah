...
identifier**, so every build sharing `io.klimov.spellchecker` reads and writes the same stored
...
and it leaves a permanent `spell-checker translate` behind.
...
`Sources/SpellCheckerCore/InputText.swift`:
...
`SpellCheckerCore`'s rule is narrower than "imports nothing" — it is **no UI frameworks in Core**.
...
**Learned in execution — imports are file-scoped.** Files in `SpellCheckerCore` declare their own
...
Text mode prints the Russian alone, so `pbpaste | spell-checker translate | pbcopy` works. Word
mode prints:
...
- **Regression:** `spell-checker check` still prints one verdict after the `ClaudeCLI` extraction.
