...
```
cli/Package.swift
  ├─ SpellCheckerCore   (library target)     Verdict, TextEvaluator, ClaudeCLIEvaluator — made `public`
  ├─ SpellChecker       (executable target)  binary: spell-checker      (the CLI; imports Core; main.swift)
  └─ SpellCheckerBar    (executable target)  binary: spell-checker-bar  (the menu-bar app; imports Core + KeyboardShortcuts)
```

- **`SpellCheckerCore`** — move `TextEvaluator.swift` and `ClaudeCLIEvaluator.swift` (and the
  `Verdict` enum) here; mark the shared types/functions `public`. Single source of truth.
- **`SpellChecker`** (CLI) — unchanged behaviour; `main.swift` now `import SpellCheckerCore`.
- **`SpellCheckerBar`** (app) — pure **AppKit** (`NSStatusItem`, `NSMenu`, `NSPasteboard`). **No
...
### Components inside `SpellCheckerBar`
...
- **`CheckCoordinator`** — orchestrates a single check: guards `isChecking`, reads the clipboard,
  calls `SpellCheckerCore`, maps the result to an icon state. A press during the display window
...
- **`make app`** — `cd cli && swift build -c release`, then assemble `dist/SpellChecker.app`:
  - `Contents/MacOS/SpellChecker` ← the `spell-checker-bar` binary
  - `Contents/Info.plist` ← `CFBundleName`, `CFBundleIdentifier` (e.g. `io.klimov.spellchecker`),
    `CFBundleExecutable=SpellChecker`, `LSUIElement=true`, `LSMinimumSystemVersion=13.0`
  - optional ad-hoc sign: `codesign --force --sign - dist/SpellChecker.app`
- **`make run-app`** — `make app` then `open dist/SpellChecker.app`.
